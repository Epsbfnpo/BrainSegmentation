...
class AgeConditionedModule(nn.Module):
    ...
    def forward(self, x, age):
        ...
        if hasattr(self, 'gamma'):
            gamma = self.gamma(age_feat).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # (B, C, 1, 1, 1)
            beta = self.beta(age_feat).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # (B, C, 1, 1, 1)
...
class SimplifiedDAUnetModule(nn.Module):
    def __init__(self, base_model: nn.Module, num_classes: int = 88,
        ...
        if use_age_conditioning:
            ...
            self.age_conditioner = AgeConditionedModule(age_embed_dim, base_model.out_channels)
            ...
            self.age_predictor = nn.Sequential(
                ...
                nn.Flatten(),
                nn.Linear(base_model.out_channels, age_embed_dim),
                nn.ReLU(),
    ...
    def compute_losses(self, source_images: torch.Tensor, source_labels: torch.Tensor,
        ...
        if self.use_age_conditioning and source_ages is not None:
