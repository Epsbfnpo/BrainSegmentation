...
def main():
    ...
    try:
        ...
        age_graph_loss = AgeConditionedGraphPriorLoss(
        ...
        if (args.prior_adj_npy and os.path.exists(args.prior_adj_npy)) or (args.weighted_adj_npy and os.path.exists(args.weighted_adj_npy) if args.weighted_adj_npy else False):
            if is_main:
                print(f"🧠 Initializing Graph Prior arguments (with age-aware extensions)…")

            # Get class weights from model if available
            actual_model = model.module if hasattr(model, 'module') else model
            class_weights = getattr(actual_model, 'class_weights', None)

            # Adjusted target weights for stability (kept from your original logic)
            ...
            graph_loss_kwargs = {
                # Target domain (binary) topology priors
                'prior_adj_path': args.prior_adj_npy,
                'required_json': args.prior_required_json,
                'forbidden_json': args.prior_forbidden_json,
                'lr_pairs_json': args.laterality_pairs_json,
                ...
                # ===== Age-aware extensions (NEW) =====
                'weighted_adj_npy': args.weighted_adj_npy,
                'age_weights_json': args.age_weights_json,
                'volume_stats_json': args.volume_stats_json,
                'shape_templates_pt': args.shape_templates_pt,
                'lambda_volume': args.lambda_volume,
                'lambda_shape': args.lambda_shape,
                'lambda_weighted_adj': args.lambda_weighted_adj,
                'lambda_topo': args.lambda_topo,
