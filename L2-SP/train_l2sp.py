...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Target-only age-aware segmentation training (L2-SP)")
    ...
    parser.add_argument("--lambda_l2sp", type=float, default=0.01, help="Weight for L2-SP regularization")
...
def main():
    ...
    class_map_path = Path(args.class_map_json).resolve() if args.class_map_json else None
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if is_main:
            seg_msg = (
                ...
                f"focal={train_metrics.get('focal', 0.0):.4f} l2sp={train_metrics.get('l2sp', 0.0):.4f}"
            )
            print(
