    ...
    for step, batch in enumerate(loader):
        ...
        with sync_context:
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if ref_model is not None and lambda_l2sp > 0:
                    reg_loss = torch.tensor(0.0, device=device)

                    def _iter_params(module: nn.Module):
                        if isinstance(module, torch.nn.parallel.DistributedDataParallel):
                            return module.module.parameters()
                        return module.parameters()

                    for param_curr, param_ref in zip(_iter_params(model), _iter_params(ref_model)):
                        if param_curr.requires_grad:
                            reg_loss = reg_loss + torch.sum((param_curr - param_ref) ** 2)
                    l2sp_loss = lambda_l2sp * 0.5 * reg_loss

                loss = seg_losses["total"] + l2sp_loss
        ...
        per_batch = {
            ...
            "l2sp": float(l2sp_loss.detach().item()),
        }
        ...
        if should_step:
            ...
            if writer is not None and is_main and (global_step % log_interval == 0):
                ...
                writer.add_scalar("train/l2sp", accum_metrics["l2sp"] / denom, global_step)
                if update_count > 0:
        ...
        if debug_mode and is_main and step < debug_step_limit:
            print(
                ...
                f"l2sp={l2sp_loss.item():.4f} grad_norm={(grad_norm_sum / max(update_count, 1)):.3f}",
                flush=True,
            )
    metrics = {}
