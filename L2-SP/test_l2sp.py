#!/usr/bin/env python3
"""
L2-SP Method Evaluation Script
- Adapted from FreqFit/TRM V9 robust testing logic.
- Uses SimplifiedDAUnetModule wrapper as defined in L2-SP training.
- Includes robust alignment (MetaTensor + Resample) and full metrics suite.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="L2-SP Evaluation")
    ...
    parser.add_argument("--output_dir", default="./test_predictions_l2sp", help="Directory for outputs")
    parser.add_argument("--metrics_path", default="./test_metrics_l2sp.json", help="JSON output for metrics")
    ...
    parser.add_argument("--use_swin_checkpoint", action="store_true")  # Checkpointing
...
# --- Model Builder for L2-SP ---

def build_l2sp_model(args, device):
    """Builds the SwinUNETR backbone wrapped in SimplifiedDAUnetModule and loads weights."""
    print("🏗️ Building L2-SP Model (SwinUNETR + Wrapper)...")
    ...
    # 2. Wrapper (L2-SP Logic)
    # We pass None for volume_stats as we are in inference mode and don't need class weights for loss
    model = SimplifiedDAUnetModule(
        backbone,
        num_classes=args.out_channels,
        volume_stats_path=None,
        foreground_only=args.foreground_only,
        enhanced_class_weights=False,
    ...
    if os.path.isfile(args.model_path):
        ...
        # L2-SP Checkpoints usually store state in 'state_dict' or 'model_state_dict'
...
def _get_inference_affine(images: torch.Tensor) -> np.ndarray:
    """Robustly extract affine from Monai MetaTensor."""
...
def _prepare_pred_volume(pred_labels: torch.Tensor, brain_mask: Optional[torch.Tensor],
    ...
    """Prepares the volume for saving (remaps back to 1..87 if needed)."""
    ...
    # Logic matching L2-SP data loader:
    # Loader maps 1..87 -> 0..86
    # We map back 0..86 -> 1..87 for visualization
    if foreground_only:
...
def evaluate(args):
    print("Running L2-SP Evaluation (Robust V9 Logic)")
    torch.manual_seed(args.seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # 1. Load Data (uses L2-SP's native loader)
    test_loader = get_target_test_loader(args, keep_meta=True)

    # 2. Build Model
    model = build_l2sp_model(args, device)
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            ...
            labels = batch["label"].to(device)  # Labels are -1 (bg), 0..86 (fg)
            ...
            with autocast(enabled=args.use_amp):
                # Pass both images and age if available, L2-SP supports age conditioning
                # SimplifiedDAUnetModule signature: forward(x, age=None)
                # Loader provides 'age' in batch
                age_tensor = batch.get("age", None)
                if age_tensor is not None: age_tensor = age_tensor.to(device)

                # Use sliding window for robust inference
                # We need a lambda or wrapper to pass 'age' if we use sliding_window_inference
                # But sliding_window_inference splits spatial dims, age is global.
                # Standard Monai inferer usually handles args via *args.
                # However, SimplifiedDAUnetModule might not propagate age if sliced.
                # We can wrap the model to bind the age for this call.

                class AgeWrapper(torch.nn.Module):
                    ...
                    def forward(self, x):
                        return self.model(x, age=self.age)

                infer_model = AgeWrapper(model, age_tensor)
            ...
            # --- Metrics Processing ---
            # Mask prediction for metrics to reduce background noise, same as FreqFit
            pred_for_metrics = torch.where(brain_mask, pred, torch.full_like(pred, -1))

            pred_clamped = pred_for_metrics.clone()
            ...
            try:
                val_hd = float(hd95_metric.aggregate().item()) if isinstance(hd95_metric.aggregate(),
                                                                             torch.Tensor) else 0.0
            except:
                val_hd = 0.0
            try:
                val_assd = float(assd_metric.aggregate().item()) if isinstance(assd_metric.aggregate(),
                                                                               torch.Tensor) else 0.0
            except:
                val_assd = 0.0

            # Advanced
            adj = adv.compute_adjacency(pred_for_metrics[0], brain_mask[0])
            viol = adv.compute_structural_violations(adj)
            rve = adv.compute_rve(pred_for_metrics[0], labels_clamped[0], brain_mask[0])
            sym = adv.compute_symmetry(pred_for_metrics[0], brain_mask[0])

            age_val = float(age_tensor.item()) if age_tensor is not None else 40.0
            spec = adv.compute_spectral_distance(adj, age_val)

            # Skeleton
            pred_np = pred_for_metrics[0].detach().cpu().numpy()
            target_np = labels_clamped[0].detach().cpu().numpy()
            cldice_sum, cbdice_sum, valid_cnt = 0.0, 0.0, 0
            # Classes are 0..86 in numpy array
            ...
            # --- Save ---
            pred_vol = _prepare_pred_volume(pred_for_metrics, brain_mask, args.foreground_only)
            ...
            # Debug: Count labels
            unique_labels = np.unique(pred_vol[pred_vol > 0])

            elapsed = time.time() - start_time
            print(
                f"[{batch_idx}] {case_id}: Dice={batch_dice:.4f}, HD95={val_hd:.2f}, RVE={rve:.4f}, Labels={len(unique_labels)}/87")
    ...
    keys = ["dice", "hd95", "assd", "rve", "symmetry", "cldice", "cbdice", "clce", "spec_distance", "label_count"]
    ...
    print(f"  Mean Label Count: {summary.get('mean_label_count', 0):.1f}/87")
    print("=" * 40)
