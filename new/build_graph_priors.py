...
def main(args):
    ...
    # === NEW: age-binned accumulators ===
    age_bin_width = getattr(args, "age_bin_width", 1.0)
    age_adj_sums = {}     # age_bin -> sum of A
    age_adj_counts = {}   # age_bin -> count
    vol_stats = defaultdict(list)  # age_bin -> list of volume fraction vectors
    ...
    for item in tqdm(files):
        ...
        if age_weeks is not None:
            ...
            if age_bin not in age_adj_sums:
                age_adj_sums[age_bin] = np.zeros_like(A, dtype=np.float64)
                age_adj_counts[age_bin] = 0
            age_adj_sums[age_bin] += A
            age_adj_counts[age_bin] += 1
    ...
    # Save a copy as weighted adjacency prior for age-aware regression
    weighted_adj_path = os.path.join(args.out_dir, "weighted_adj.npy")
    np.save(weighted_adj_path, A_prior_norm)
    print(f"Saved weighted adjacency (same as normalized adj) to: {weighted_adj_path}")
    ...
    # === NEW: dump age-binned priors ===
    # (1) volume_stats.json  {age_bin: {"means":[...], "stds":[...]}}
    vol_out = {}
    for age_bin, rows in vol_stats.items():
        arr = np.asarray(rows, dtype=np.float64)
        vol_out[str(age_bin)] = {
            "means": arr.mean(0).tolist(),
            "stds":  (arr.std(0) + 1e-8).tolist()
        }

    out_vs_json = args.out_volume_stats_json or os.path.join(args.out_dir, "volume_stats.json")
    with open(out_vs_json, "w") as f:
        json.dump(vol_out, f, indent=2)
    print(f"Saved volume stats to: {out_vs_json}")

    # (2) age_weights.json  {age_bin: [[C x C]]}  (行归一化的邻接，作为年龄权重)
    age_weights = {}
    for age_bin, A_sum_bin in age_adj_sums.items():
        cnt = max(1, age_adj_counts.get(age_bin, 1))
        A_bin = A_sum_bin / cnt
        rs = A_bin.sum(axis=1, keepdims=True)
        rs[rs == 0] = 1.0
        A_bin_norm = A_bin / rs
        age_weights[str(age_bin)] = A_bin_norm.tolist()

    out_aw_json = args.out_age_weights_json or os.path.join(args.out_dir, "age_weights.json")
    with open(out_aw_json, "w") as f:
        json.dump(age_weights, f, indent=2)
    print(f"Saved age-dependent adjacency weights to: {out_aw_json}")
    ...
    with open(report_path, 'w') as f:
        ...
        for k, (prob, i, j) in enumerate(adj_pairs[:10]):
            ...
            mask_status = "✓" if R_mask[i, j] > 0 else "✗"
    ...
    print("\n✅ Prior generation complete with restricted mask!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--age_bin_width", type=float, default=1.0,
                        help="Width of age bin in weeks (default: 1.0)")
    parser.add_argument("--out_volume_stats_json", type=str, default=None,
                        help="Path to write volume_stats.json (defaults to <out_dir>/volume_stats.json)")
    parser.add_argument("--out_age_weights_json", type=str, default=None,
                        help="Path to write age_weights.json (defaults to <out_dir>/age_weights.json)")
