    ...
    def forward(self,
        ...
        if self.lambda_weighted_adj > 0:
            ...
            if self.A_weighted_prior.numel() > 0:
                ...
                with torch.no_grad():
                    predicted_volumes = probs.detach().float().sum(dim=(2, 3, 4))
                    total_volume = predicted_volumes.sum(dim=1, keepdim=True).clamp(min=1e-6)
                    volume_fractions = (predicted_volumes / total_volume).to(device=device)
                    presence_mask = (volume_fractions > self.weighted_adj_presence_threshold).to(
                        device=device,
                        dtype=dtype,
                    )
                ...
                for b in range(B):
                    presence = presence_mask[b]
                    active_classes = int((presence > 0).sum().item())
                    active_counts.append(active_classes)
                    if active_classes <= 1:
                        continue
                    mask = torch.outer(presence, presence)
                    mask = mask * (1 - eye)
                    if mask.sum() <= 0:
                        continue
                    ...
                    vol_weight = torch.outer(
                        volume_fractions[b].to(device=device, dtype=dtype),
                        volume_fractions[b].to(device=device, dtype=dtype),
                    )
