...
class DistributedWeightedSampler(Sampler[int]):
    ...
    def __iter__(self):
        ...
        indices = torch.multinomial(probs, self.total_size, self.replacement, generator=generator)
...
def _augmentation_transforms(args, *, laterality_pairs: Optional[Sequence[Tuple[int, int]]] = None) -> Compose:
...
def _base_transforms(
    ...
    *,
    ...
    if mode == "train":
        ...
        transforms.append(EnsureTyped(keys=["image", "label", "image_clean", "label_clean"], track_meta=False))
...
def _inference_transforms(args, *, keep_meta: bool = True) -> Compose:
    ...
    if getattr(args, "foreground_only", False):
        transforms.append(RemapLabelsd(keys=["label"]))
    transforms.append(EnsureTyped(keys=["image", "label"], track_meta=keep_meta))
    return Compose(transforms)
...
def _load_class_ratios(args, *, is_main: bool) -> Optional[List[float]]:
    ...
    if ratios.sum() <= 0:
        if is_main:
            print("  ⚠️  Invalid ratios detected; falling back to uniform")
        return None
...
def get_inference_loader(args) -> TorchDataLoader:
    items = _load_test_split(args.split_json)
    items = _process_items(items)
    transform = _inference_transforms(args, keep_meta=True)
    dataset = Dataset(items, transform=transform)
    return DataLoader(dataset, batch_size=1, shuffle=False, num_workers=max(1, args.num_workers // 2))
