    ...
    for step, batch in enumerate(loader):
        ...
        for key in sum_metrics:
            if key == "loss":
                ...
                value = float(seg_losses["total"].detach().item())
            else:
                value = float(seg_losses[key].detach().item())
        ...
        if should_step:
            ...
            if writer is not None and is_main and (global_step % log_interval == 0):
                ...
                writer.add_scalar("train/dice_loss", accum_metrics["dice"] / denom, global_step)
                writer.add_scalar("train/ce_loss", accum_metrics["ce"] / denom, global_step)
                writer.add_scalar("train/focal_loss", accum_metrics["focal"] / denom, global_step)
                if update_count > 0:
        ...
        for step, batch in enumerate(loader):
            ...
            for b_idx in range(len(pred_np_batch)):
                ...
                for cls in range(1, num_classes):
                    p_cls = (p_vol == cls).astype(np.uint8)
                    t_cls = (t_vol == cls).astype(np.uint8)
                    if p_cls.sum() == 0 and t_cls.sum() == 0:
                        continue
                    cldice_sum += compute_cldice(p_cls, t_cls)
                    cbdice_sum += compute_cbdice(p_cls, t_cls)
                    valid_count += 1
                if valid_count > 0:
                    batch_cldice += cldice_sum / valid_count
                    batch_cbdice += cbdice_sum / valid_count
