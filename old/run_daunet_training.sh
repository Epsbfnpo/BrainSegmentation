...
LATERALITY_PAIRS_JSON="/datasets/work/hb-nhmrc-dhcp/work/liu275/GraphAlignFlex/dhcp_lr_swap.json"

# ========== TARGET DOMAIN GRAPH PRIORS ==========
TARGET_PRIORS_DIR="/datasets/work/hb-nhmrc-dhcp/work/liu275/GraphAlignFlex/priors/PPREMOPREBO"
...
SOURCE_PRIORS_DIR="/datasets/work/hb-nhmrc-dhcp/work/liu275/GraphAlignFlex/priors/dHCP"
...
if [ ! -f "$SOURCE_PRIOR_ADJ_NPY" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "âŒ Failed to build source domain graph priors"
        exit 1
    fi
...
if [ ! -f "$TARGET_PRIOR_ADJ_NPY" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "âŒ Failed to build target domain graph priors"
        exit 1
    fi
...
torchrun --standalone --nproc_per_node=$NUM_GPUS main_daunet.py \
    --epochs=$EPOCHS \
...
" 2>/dev/null
        ...
        if [ -n "$SLURM_JOB_ID" ] && [ ! -f "${RESULTS_DIR}/final_model.pth" ]; then
            echo ""
            echo "ðŸ”„ Auto-resubmitting next chunk..."
            sbatch "${SLURM_SUBMIT_DIR}/run_graph_align_flex.sbatch"
        fi
    fi
else
    echo "âŒ TRAINING FAILED (exit code: $EXIT_STATUS)"
    echo "=============================================================="
    echo ""
    ...
    if [ -n "$SLURM_JOB_ID" ] && [ -f "${RESULTS_DIR}/latest.pth" ] && [ ! -f "${RESULTS_DIR}/final_model.pth" ]; then
        echo ""
        echo "ðŸ”„ Found checkpoint, attempting to resubmit for recovery..."
        sbatch "${SLURM_SUBMIT_DIR}/run_graph_align_flex.sbatch"
    fi
fi
