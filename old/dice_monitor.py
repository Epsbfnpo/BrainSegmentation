...
class DiceMonitor:
    ...
    def add_dice_score(self, epoch: int, dice_score: float,
        ...
        if self.graph_prior_enabled:
            self.dice_history['graph_losses'].append(train_metrics.get('graph_loss', 0))
            self.dice_history['graph_spec_losses'].append(train_metrics.get('graph_spec', 0))
            self.dice_history['graph_edge_losses'].append(train_metrics.get('graph_edge', 0))
    ...
    def generate_report(self) -> None:
        ...
        with open(self.report_path, 'w') as f:
            ...
            if self.dual_branch_enabled and self.dice_history['dyn_lambda_values']:
                f.write("Epoch | Dice Score | Seg Loss | Graph(P) | Graph(D) | Î»_dyn | LR\n")
                f.write("-" * 75 + "\n")
            ...
            if self.dual_branch_enabled and self.dice_history['dyn_spec_losses']:
                ...
                if len(active_idx) > 0:
                    ...
                    f.write(f"  Current Î»_dyn: {dyn_lambda[-1]:.4f}\n")
            ...
            if len(dice_scores) >= 10:
                ...
                f.write(f"Last 10 epochs trend: {'â†‘' if trend > 0 else 'â†“'} ({trend:.5f} per epoch)\n")
        ...
        print(f"ðŸ“„ Dice monitoring report saved to: {self.report_path}")
    ...
    def generate_cross_domain_report(self) -> None:
        ...
        with open(self.crossdomain_report_path, 'w') as f:
            ...
            if len(spec_src) > window * 2:
                ...
                if src_ratio > 5:
                    f.write("  âš ï¸ Source alignment may be dominating\n")
                elif src_ratio < 0.2:
                    f.write("  âš ï¸ Target regularization may be too strong\n")
                else:
                    f.write("  âœ“ Balanced alignment\n")
            ...
            if len(spec_src) > 10:
                ...
                if recent_src > 0.5:
                    f.write("â€¢ Source alignment loss is high - consider:\n")
                ...
                if recent_tgt < 0.01:
                    f.write("â€¢ Target regularization is very weak - consider:\n")
                ...
                if len(dice_scores) == len(spec_src):
                    ...
                    if corr > 0.7:
                        f.write(f"â€¢ High positive correlation ({corr:.2f}) between dice and source loss\n")
                        f.write("  - Source alignment may be hindering segmentation\n")
                        f.write("  - Consider reducing source alignment weights\n")

        print(f"ðŸ“Š Cross-domain report saved to: {self.crossdomain_report_path}")
    ...
    def generate_dual_branch_report(self) -> None:
        ...
        with open(self.dual_branch_report_path, 'w') as f:
            ...
            if self.dice_history['forbidden_ema']:
                ...
                f.write(f"  Trend: {'â†‘' if forbidden_ema[-1] > forbidden_ema[-10] else 'â†“'}\n")
            ...
            if self.dice_history['dyn_spec_losses'] and len(dyn_spec) > 20:
                ...
                if recent_dyn > 0.1:
                    f.write("â€¢ Dynamic loss is high - consider:\n")
                ...
                if np.sum(dyn_conflicts[-10:]) > 3:
                    f.write("â€¢ Frequent conflicts detected - consider:\n")
            ...
            if self.dice_history['bottom_10_dice']:
                if bottom_10[-1] < 0.5:
                    f.write("â€¢ Small structures underperforming - consider:\n")
                    f.write("  - Increasing qap_mismatch_g for stronger penalties\n")
                    f.write("  - Adjusting class weights more aggressively\n")

        print(f"ðŸ“Š Dual-branch report saved to: {self.dual_branch_report_path}")
    ...
    def plot_dice_evolution(self) -> None:
        ...
        print(f"ðŸ“ˆ Dice evolution plot saved to: {plot_path}")
