        ...
        for key in self.key_iterator(d):
            ...
            if unique_before.max() > 87 or unique_before.min() < 0:
                if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
                    print(f"âš ï¸  WARNING: Labels out of expected range!")
...
def _load_lr_pairs(args):
    ...
    if getattr(args, 'laterality_pairs_json', None) and os.path.exists(args.laterality_pairs_json):
        ...
        if is_main:
            print(f"\nðŸ“‹ Loaded {len(pairs)} laterality pairs")
            if len(pairs) > 0:
...
def get_weighted_ratios_for_small_classes(class_prior_path: str, num_small_classes: int = 20,
    ...
    is_main = not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0
    if is_main:
        print(f"\nðŸ“Š Weighted sampling ratios computed:")
...
def get_cache_transforms(args, is_registered=False):
    """Get transforms for cached dataset (train)"""
    transforms = [
...
def get_source_target_dataloaders(args, is_distributed=False, world_size=1, rank=0):
    try:
        ...
        if torch.cuda.is_available() and is_main:
            print(f"\nðŸ–¥ï¸  Initial GPU memory: {torch.cuda.memory_allocated(rank) / 1024 ** 3:.2f} GB allocated")
        ...
        if is_main:
            print(f"\nðŸ“Š Data Registration Status (from CLI args):")
        ...
        if is_main:
            print(f"\nðŸ“‚ Loading source split from: {args.source_split_json}")
        ...
        if is_main:
            print(f"ðŸ“‚ Loading target split from: {args.split_json}")
        ...
        if is_main:
            print("\nðŸŽ¨ Creating transforms...")
            print("  ðŸ“¦ Cache transforms: Load + Spacing + Orient + Normalize + Remap + Crop")
            if is_source_registered or is_target_registered:
                print("  ðŸŽ² Runtime augmentations: Laterality-aware flips + mild intensity")
            else:
                print("  ðŸŽ² Runtime augmentations: Full (Laterality-aware flips + Rotations + Intensity)")
        ...
        if use_cache:
            ...
            if should_preshard:
                if is_main:
                    print(f"\nðŸ”ª Pre-sharding training data for distributed caching")
                ...
                print(
                    f"  ðŸŽ¯ Rank {rank}: caching {len(source_train_files_sharded)}/{len(source_train_files)} source samples")
                print(
                    f"  ðŸŽ¯ Rank {rank}: caching {len(target_train_files_sharded)}/{len(target_train_files)} target samples")
                ...
                if is_main:
                    ...
                    if args.cache_rate == 1.0:
                        print(f"  âš ï¸  WARNING: Each GPU will cache ALL data (no pre-sharding)")
                        print(f"  âš ï¸  This may cause OOM. Consider using distributed mode.")
        ...
        if is_main:
            print("\nðŸ“¦ Creating source training dataset (cache)...")
        ...
        if is_main:
            print("ðŸ“¦ Creating target training dataset (cache)...")
        ...
        if is_main:
            print("ðŸŽ² Wrapping with augmentation layer...")
        ...
        if is_main:
            print("ðŸ“Š Creating validation datasets (no cache)...")
        ...
        if is_main:
            ...
            if is_distributed:
                ...
                if args.cache_rate == 1.0 and use_cache:
                    if should_preshard:
                        print(f"  âœ¨ Memory optimization: Each GPU caches ~{100 / world_size:.1f}% of training data")
                        print(f"  âœ¨ Total coverage: 100% (distributed across {world_size} GPUs)")
                    else:
                        print(f"  âš ï¸  No memory optimization: Each GPU caches 100% of data")
            ...
            print(f"  âœ¨ Weighted sampling enabled for small classes")
            print(f"  âœ¨ Laterality-aware augmentations enabled")
        ...
        if is_main:
            print(f"\nâŒ Error creating data loaders: {str(e)}")
            traceback.print_exc()
        raise