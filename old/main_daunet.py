...
from ada_components_daunet import SimplifiedDAUnetModule
from daunet_trainer import (
    train_epoch_simplified, val_epoch_simplified, save_checkpoint_simplified
)
from data_loader_daunet import get_source_target_dataloaders
...
def load_pretrained_model(model, checkpoint_path, device):
    ...
    if is_main:
        print(f"\nðŸ”¥ Loading pretrained model from: {checkpoint_path}")
...
def main():
    ...
    try:
        ...
        dice_monitor = DiceMonitor(
            ...
            graph_prior_enabled=(args.prior_adj_npy is not None),
            cross_domain_enabled=(args.src_prior_adj_npy is not None),
            dual_branch_enabled=True  # NEW: track dual-branch metrics
        ...
        if is_main:
            ...
            with open(config_path, 'w') as f:
                ...
                config_dict['version'] = 'DUAL_BRANCH_CROSS_DOMAIN_GRAPH_ALIGNMENT_V2'
                config_dict['graph_prior_enabled'] = (args.prior_adj_npy is not None)
                config_dict['cross_domain_alignment'] = (args.src_prior_adj_npy is not None)
                config_dict['dynamic_spectral_enabled'] = True
                json.dump(config_dict, f, indent=2)

            print("\n" + "=" * 80)
            print("DAUnet TRAINING - WITH DUAL-BRANCH CROSS-DOMAIN GRAPH ALIGNMENT")
            print("=" * 80)
            ...
            # Graph prior info
            if args.prior_adj_npy:
                print("\nðŸ§  TARGET DOMAIN GRAPH PRIORS")
                print(f"  Adjacency matrix: {args.prior_adj_npy}")
                if args.prior_required_json:
                    print(f"  Required edges: {args.prior_required_json}")
                if args.prior_forbidden_json:
                    print(f"  Forbidden edges: {args.prior_forbidden_json}")

            if args.src_prior_adj_npy:
                print("\nðŸŽ¯ SOURCE DOMAIN GRAPH PRIORS (CROSS-DOMAIN ALIGNMENT)")
                print(f"  Adjacency matrix: {args.src_prior_adj_npy}")
                if args.src_prior_required_json:
                    print(f"  Required edges: {args.src_prior_required_json}")
                if args.src_prior_forbidden_json:
                    print(f"  Forbidden edges: {args.src_prior_forbidden_json}")
                print(f"\n  Alignment Mode: {args.graph_align_mode}")
                print(f"  Lambda spectral (source): {args.lambda_spec_src or args.lambda_spec}")
                print(f"  Lambda edge (source): {args.lambda_edge_src or args.lambda_edge}")
                print(f"  Lambda spectral (target): {args.lambda_spec_tgt or args.lambda_spec * 0.3}")
                print(f"  Lambda edge (target): {args.lambda_edge_tgt or args.lambda_edge * 0.3}")
                ...
                # NEW: Dynamic alignment parameters
                print(f"\nðŸ”„ DYNAMIC SPECTRAL ALIGNMENT")
        ...
        # Create simplified DAUnet module
        if is_main:
            print("\nðŸ— Creating simplified DAUnet module...")
        ...
        # === CREATE ENHANCED GRAPH PRIOR LOSS WITH DUAL-BRANCH ALIGNMENT ===
        if args.prior_adj_npy and os.path.exists(args.prior_adj_npy):
            if is_main:
                print(f"\nðŸ§  Initializing Dual-Branch Graph Prior Loss...")

            from graph_prior_loss import GraphPriorLoss

            # Get class weights from model if available
            actual_model = model.module if hasattr(model, 'module') else model
            class_weights = actual_model.class_weights if hasattr(actual_model, 'class_weights') else None

            # Adjust weights for target domain based on recommendations
            ...
            graph_loss_kwargs = {
                ...
                'warmup_epochs': args.graph_warmup_epochs,
                ...
                # NEW: Dynamic alignment parameters (passed to trainer)
            ...
            # Store in args for trainer to use
            args.graph_loss_kwargs = graph_loss_kwargs

            if is_main:
                print(f"  âœ” Dual-branch graph loss configured")
                print(f"    Prior branch: stable structural anchoring")
                print(f"    Dynamic branch: flexible spectral alignment")
                if args.use_restricted_mask:
                    print(f"    Restricted mask: enabled for conflict resolution")
                if class_weights is not None:
                    print(f"    Class weights: enabled for edge reweighting")
        else:
            args.graph_loss_kwargs = None
        ...
        if args.resume_training:
            ...
            if checkpoint:
                if is_main:
                    print(f"\nðŸ”„ Loading checkpoint from: {loaded_path}")
        ...
        def save_latest_checkpoint(reason="signal"):
            ...
            try:
                if is_main:
                    ...
                    if success:
                        print(f"\nðŸ›Ÿ Saved latest checkpoint at epoch {current_epoch_holder['value']} due to {reason}")
                    else:
                        print(f"\nâš ï¸ Failed to save checkpoint due to {reason}")

            except Exception as e:
                if is_main:
                    print(f"âŒ Failed to save checkpoint on {reason}: {e}")
        ...
        def sigterm_handler(signum, frame):
            ...
            if is_main:
                ...
                if job_id:
                    print(f"\nðŸ” Received SIGTERM for job {job_id}")
                    ...
                    print("\nðŸ” Received SIGTERM (no SLURM_JOB_ID)")
        ...
        if is_dist():
            if is_main:
                print("\nðŸ”„ Synchronizing all processes before training...")
            dist.barrier()
            if is_main:
        ...
        # Training loop
        if is_main:
            print(f"\nðŸš€ Starting training from epoch {start_epoch} to {args.epochs}...")
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            if time_manager.should_stop_training():
                if is_main:
                    print(f"\nâ° Approaching job time limit, saving checkpoint and exiting gracefully...")
                    time_manager.print_status(is_main)
            ...
            if not time_manager.can_complete_epoch(will_validate):
                if is_main:
                    print(f"\nâ° Not enough time to complete epoch {epoch}, saving checkpoint...")
                    time_manager.print_status(is_main)
                save_latest_checkpoint("time_limit")
                exit_reason = "time_limit"
                break
            ...
            try:
                ...
                # Train one epoch (with enhanced graph loss kwargs)
                ...
                if epoch % args.eval_num == 0:
                    ...
                    if not time_manager.can_complete_validation():
                        if is_main:
                            print(f"\nâ° Not enough time for validation, saving checkpoint...")
                            time_manager.print_status(is_main)
                    ...
                    val_start = time.time()
                    val_metrics = val_epoch_simplified(
                        model=model,
                        loader=target_val_loader,
                        epoch=epoch,
                        writer=writer,
                        args=args,
                        dice_monitor=dice_monitor,
                        device=device,
                        is_distributed=is_dist(),
                        world_size=world_size,
                        rank=local_rank
                ...
                if is_main:
                    print(f"\nâŒ Error in epoch {epoch}: {str(e)}")
                    traceback.print_exc()
        ...
        if is_main:
            if training_completed:
                ...
                if exit_reason == "time_limit":
                    ...
                    print("âŒ TRAINING STOPPED DUE TO ERROR")
                    print(f"Stopped at epoch {final_epoch}/{args.epochs}")
                else:
        ...
        is_main = (not is_dist()) or dist.get_rank() == 0
        if is_main:
            print(f"\nâŒ Training failed with error: {str(e)}")
            traceback.print_exc()
            # Try to save emergency checkpoint
            try:
                save_latest_checkpoint("crash")
            except:
                pass
        if is_dist():
            dist.destroy_process_group()
        ...
        try:
            with open(err_path, "w") as f:
                ...
                if "out of memory" in str(e).lower():
                    f.write("\nMemory Error - Current Memory Status:\n")
                    if torch.cuda.is_available():
                        try:
            ...
            print(f"\nâŒ [Rank {rank}] CRASHED with {type(e).__name__}: {str(e)}")
            print(f"   Full traceback saved to: {err_path}")

        except Exception as write_error:
            # If we can't even write the error file, at least print it
            print(f"\nâŒ [Rank {rank}] CRASHED and FAILED TO WRITE ERROR LOG: {write_error}")
