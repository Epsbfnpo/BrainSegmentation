...
class AgeConditionedGraphPriorLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if volume_stats_path and os.path.exists(volume_stats_path):
            with open(volume_stats_path, 'r') as f:
                tmp_vs = json.load(f)  # 键是字符串
                self.volume_stats = {float(k): v for k, v in tmp_vs.items()}  # 转回 float
        ...
        if age_weights_path and os.path.exists(age_weights_path):
            with open(age_weights_path, 'r') as f:
                tmp_aw = json.load(f)  # 键是字符串
                self.age_weights = {float(k): v for k, v in tmp_aw.items()}  # 转回 float
    ...
    def forward(self, logits: torch.Tensor, labels: torch.Tensor,
        ...
        if self.lambda_weighted_adj > 0:
            # (a) 先计算“未加权”的预测邻接
            A_pred = compute_weighted_adjacency(probs, age, None, self.temperature)  # no age_weights here

            if self.A_weighted_prior is not None:
                # (b) 按样本的年龄取权重矩阵 W(age)，做加权 MSE
                B, C, _ = A_pred.shape
                loss_adj = 0.0
                for b in range(B):
                    if self.age_weights is not None:
                        W = get_age_weight_matrix(age[b].item(), self.age_weights, C, probs.device)
                    else:
                        W = torch.ones(C, C, device=probs.device)
                    diff = (A_pred[b] - self.A_weighted_prior)
                    loss_adj += (W * diff * diff).mean()
